...
def generate_node2vec_embeddings(A, emd_size=128, negative_injection=False, train_neg=None):
    if negative_injection:
        ...
        # A[col, row] = 1  # inject negative train, this one not necessary??
...
def generate_CANE_embeddings(A, text):
    ...
    for array in text:
        for item in array:
            for item2 in item:
                string_list.append(item2)

    string_with_commas = ', '.join(string_list) 
    # Save the string to a text file
    with open(file_path, 'w') as file:
        file.write(string_with_commas)
   
    file_path_network = 'data/network.txt'
    with open(file_path_network, 'w') as file:
        file.write(str(A_list))

    subprocess.run(["python3", "train.py", "--net", file_path_network, "--text", file_path])
    array = np.loadtxt('temp/embed.txt')
    return array
...
def calc_recall(pred_name, threshold):
    # Load the predicted links
    predicted_links = np.loadtxt(pred_name).astype(float)

    # Get the true positives and false negatives
    true_positives = np.sum(predicted_links[:, 2] > threshold)
    false_negatives = predicted_links.shape[0] - true_positives
